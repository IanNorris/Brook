/*---------------------------------------------------------------------------/
/  FatFs Module Configuration — Brook OS overlay
/
/  This file shadows the upstream ffconf.h from src/third_party/fatfs/ so we
/  can customise settings without modifying the submodule.  Keep it in sync
/  with the upstream version when updating FatFS.
/
/  Brook-specific changes vs upstream defaults:
/    FF_USE_LFN  0 → 1  (enable long file names, static buffer)
/    FF_VOLUMES  1 → 4  (ramdisk + virtio + future expansion)
/---------------------------------------------------------------------------*/
