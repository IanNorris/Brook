...
if [ -f "${YQ2_WRAP}" ]; then
    ...
    # Optional libxkbcommon interposer: BRO216_XKBSPY=1 preloads xkbspy.so so
    # SDL3's libxkbcommon calls are logged to serial (pins where text input
    # fails without rebuilding SDL3). Off by default (clean run).
    XKBSPY_LINE=""
    if [ "${BRO216_XKBSPY:-0}" = "1" ]; then
        XKBSPY_LINE="export LD_PRELOAD=/nix/store/${PROBE_BASE}/lib/xkbspy.so"
        echo "  (BRO216_XKBSPY=1: kbdprobe-play.sh will LD_PRELOAD xkbspy.so)"
    fi
    {
        echo "${SHEBANG}"
        ...
        [ -n "${XKBSPY_LINE}" ] && echo "${XKBSPY_LINE}"
