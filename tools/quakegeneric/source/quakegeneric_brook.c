...
//
// Provides framebuffer output (palette→RGBA), PS/2 keyboard input, and timing.
// Based on the doomgeneric_brook.c pattern.
...
static unsigned int offset_x = 0;
static unsigned int offset_y = 0;
...
static void poll_keyboard(void)
{
    ...
    while (read(kb_fd, &sc, 1) > 0)
    {
        ...
        if (next == kq_read) continue; // queue full
        key_queue[kq_write].down = !released;
...
void QG_Init(void)
{
    ...
    if (fb_fd >= 0)
    {
        ...
        if (ioctl(fb_fd, FBIOGET_VSCREENINFO, &vinfo) == 0)
        {
            ...
            if (screen_w > QUAKEGENERIC_RES_X)
                offset_x = (screen_w - QUAKEGENERIC_RES_X) / 2;
            if (screen_h > QUAKEGENERIC_RES_Y)
                offset_y = (screen_h - QUAKEGENERIC_RES_Y) / 2;
        ...
        fprintf(stderr, "Brook Quake: fb %ux%u, render at (%u,%u)\n",
...
void QG_DrawFrame(void *pixels)
{
    ...
    // 2x scale: 320×240 → 640×480, clipped to screen height
    int scale = 2;
    int draw_w = QUAKEGENERIC_RES_X * scale;
    int draw_h = QUAKEGENERIC_RES_Y * scale;
    if (draw_w > (int)screen_w) draw_w = (int)screen_w;
    if (draw_h > (int)screen_h) draw_h = (int)screen_h;
    ...
    if (!debug_once) {
        fprintf(stderr, "QG_DrawFrame: %ux%u scale=%d\n",
